...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter(p => terrain(p._1.b1) && terrain(p._1.b2))

    /**
     * Returns `true` if the block is standing.
     *
     * Jeśli zajmują te same współrzędne
     */
     ...
     *
     * oba klocki muszą zanjdować się w obrębie terenu.
     */
